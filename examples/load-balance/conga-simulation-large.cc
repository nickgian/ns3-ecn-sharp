...
void TLBPathSelectTrace(uint32_t flowId, uint32_t fromTor, uint32_t destTor,
                        uint32_t path, bool isRandom, PathInfo pathInfo,
                        std::vector<PathInfo> parallelPaths) {
  NS_LOG_UNCOND("Flow: " << flowId << " (" << fromTor << " -> " << destTor
                         << ") selects path: " << path
                         << " at: " << Simulator::Now());
  NS_LOG_UNCOND("\t Is random select: " << isRandom);
  NS_LOG_UNCOND("\t Path info: type -> "
                << Ipv4TLB::GetPathType(pathInfo.pathType) << ", min RTT -> "
                << pathInfo.rttMin << ", ECN Portion -> " << pathInfo.ecnPortion
                << ", Flow counter -> " << pathInfo.counter
  ...
  for (; itr != parallelPaths.end(); ++itr) {
    ...
    NS_LOG_UNCOND("\t\t Path info: "
                  << path.pathId << ", type -> "
                  << Ipv4TLB::GetPathType(path.pathType) << ", min RTT -> "
                  << path.rttMin << ", Path size -> " << path.size
                  << ", ECN Portion -> " << path.ecnPortion
                  << ", Flow counter -> " << path.counter
                  << ", Quantified DRE -> " << path.quantifiedDre);
  }
  NS_LOG_UNCOND("\n");

  std::ofstream out(tlbBibleFilename.str().c_str(),
                    std::ios::out | std::ios::app);
  out << "Flow: " << flowId << " (" << fromTor << " -> " << destTor
      << ") selects path: " << path << " at: " << Simulator::Now() << std::endl;
  out << "\t Is random select: " << isRandom << std::endl;
  out << "\t Path info: type -> " << Ipv4TLB::GetPathType(pathInfo.pathType)
      << ", min RTT -> " << pathInfo.rttMin << ", ECN Portion -> "
      << pathInfo.ecnPortion << ", Flow counter -> " << pathInfo.counter
  ...
  for (; itr != parallelPaths.end(); ++itr) {
    struct PathInfo path = *itr;
    out << "\t\t Path info: " << path.pathId << ", type -> "
        << Ipv4TLB::GetPathType(path.pathType) << ", min RTT -> " << path.rttMin
        << ", Path size -> " << path.size << ", ECN Portion -> "
        << path.ecnPortion << ", Flow counter -> " << path.counter
        << ", Quantified DRE -> " << path.quantifiedDre << std::endl;
  }
  out << std::endl;
}

void TLBPathChangeTrace(uint32_t flowId, uint32_t fromTor, uint32_t destTor,
                        uint32_t newPath, uint32_t oldPath, bool isRandom,
                        std::vector<PathInfo> parallelPaths) {
  NS_LOG_UNCOND("Flow: " << flowId << " (" << fromTor << " -> " << destTor
                         << ") changes path from: " << oldPath << " to "
                         << newPath << " at: " << Simulator::Now());
  NS_LOG_UNCOND("\t Is random select: " << isRandom);
  NS_LOG_UNCOND("\t Parallel path info: ");
  std::vector<PathInfo>::iterator itr = parallelPaths.begin();
  for (; itr != parallelPaths.end(); ++itr) {
  ...
  std::ofstream out(tlbBibleFilename2.str().c_str(),
                    std::ios::out | std::ios::app);
  out << "Flow: " << flowId << " (" << fromTor << " -> " << destTor
      << ") changes path from: " << oldPath << " to " << newPath
      << " at: " << Simulator::Now();
  out << "\t Is random select: " << isRandom << std::endl;
  out << "\t Parallel path info: " << std::endl;
  itr = parallelPaths.begin();
...
void RBTraceBuffer(uint32_t flowId, Time time, SequenceNumber32 revSeq,
                   SequenceNumber32 expectSeq) {
  NS_LOG_UNCOND("Flow: " << flowId << " (at time: " << time << "), receives: "
                         << revSeq << ", while expecting: " << expectSeq);
  std::ofstream out(rbTraceFilename.str().c_str(),
                    std::ios::out | std::ios::app);
  out << "Flow: " << flowId << " (at time: " << time
      << "), receives: " << revSeq << ", while expecting: " << expectSeq
      << std::endl;
}

void RBTraceFlush(uint32_t flowId, Time time, SequenceNumber32 popSeq,
                  uint32_t inOrderQueueLength, uint32_t outOrderQueueLength,
                  TcpRBPopReason reason) {
  NS_LOG_UNCOND("Flow: " << flowId << " (at time: " << time
                         << "), pops: " << popSeq
                         << ", with in order queue: " << inOrderQueueLength
                         << ", out order queue: " << outOrderQueueLength
                         << ", reason: " << reason);
  std::ofstream out(rbTraceFilename.str().c_str(),
                    std::ios::out | std::ios::app);
  out << "Flow: " << flowId << " (at time: " << time << "), pops: " << popSeq
      << ", with in order queue: " << inOrderQueueLength
...
void RBTrace(void) {
  Config::ConnectWithoutContext(
      "/NodeList/*/$ns3::TcpL4Protocol/SocketList/*/ResequenceBufferPointer/"
      "Buffer",
      MakeCallback(&RBTraceBuffer));
  Config::ConnectWithoutContext(
      "/NodeList/*/$ns3::TcpL4Protocol/SocketList/*/ResequenceBufferPointer/"
      "Flush",
      MakeCallback(&RBTraceFlush));
}

// Port from Traffic Generator
// Acknowledged to
// https://github.com/HKUST-SING/TrafficGenerator/blob/master/src/common/common.c
...
void setSourceAttributes(BulkSendHelper source, uint32_t applicationPauseThresh,
                         uint32_t applicationPauseTime, int flowSize) {
  source.SetAttribute("SendSize", UintegerValue(PACKET_SIZE));
  source.SetAttribute("MaxBytes", UintegerValue(flowSize));
  // source.SetAttribute("DelayThresh", UintegerValue(applicationPauseThresh));
  // source.SetAttribute("DelayTime",
  // TimeValue(MicroSeconds(applicationPauseTime)));
}

void sendFlow(int fromLeafId, NodeContainer servers, int fromServerIndex,
              double startTime, long &flowCount, long &totalFlowSize,
              int SERVER_COUNT, int LEAF_COUNT, double START_TIME,
              double END_TIME, struct cdf_table *cdfTable, uint32_t flowSize,
              uint32_t applicationPauseTime, uint32_t applicationPauseThresh) {
  flowCount++;
  ...
  BulkSendHelper source = BulkSendHelper("ns3::TcpSocketFactory",
...
void install_applications(int fromLeafId, NodeContainer servers,
                          double requestRate, struct cdf_table *cdfTable,
                          long &flowCount, long &totalFlowSize,
                          int SERVER_COUNT, int LEAF_COUNT, double START_TIME,
                          double END_TIME, double FLOW_LAUNCH_END_TIME,
                          uint32_t applicationPauseThresh,
                          uint32_t applicationPauseTime,
                          std::set<int> compromised, int hostileFlowSize,
                          RunMode attackerMode, uint32_t offTime,
                          uint32_t onTime, int rate, int hostileFlows, std::string attackerProt, std::string attackerApp) {
  ...
  for (int i = 0; i < SERVER_COUNT; i++) {
    ...
    uint32_t flowSize;
    /* If it's a compromised server start one huge flow */
    if (compromised.count(fromServerIndex)) {
      if (attackerMode == LetFlow) {
        /* For letflow, the goal of the attacker is to create fake
            flowlet gaps to cause rerouting even if there is no
            congestion. Right now, I don't know if it's possible to
            detect congestion and then react in the simulator, so we do
            it in a fixed way by pausing the application for
            attackerPauseTime every attackerPauseThresh packets.*/
        flowSize = hostileFlowSize;
      } else {
        // A naive attacker just sends a large flow.
        flowSize = hostileFlowSize;
        offTime = 0;
        onTime = END_TIME * 1000000;  // Set it to simulation end time.
      }
      for (int i = 0; i < hostileFlows; i++) {
        if (attackerApp.compare("Bulk") == 0) {
              ...
              END_TIME, cdfTable, flowSize, offTime, onTime);
        }
        else {
...
RunMode parseRunMode(std::string runModeStr, int LINK_COUNT, bool asymCapacity,
                     bool asymCapacity2) {
  if (runModeStr.compare("Conga") == 0) {
    ...
    if (asymCapacity == false && asymCapacity2 == false) {
      NS_LOG_ERROR(
          "The Weighted-Presto has to work with asymmetric topology. For a "
          "symmetric topology, please use Presto instead");
      exit(0);
    }
    ...
    NS_LOG_ERROR(
        "The running mode should be TLB, Conga, Conga-flow, Conga-ECMP, "
        "Presto, FlowBender, DRB and ECMP");
    exit(0);
  }
}
...
int main(int argc, char *argv[]) {
  ...
  std::string attackerModeStr = "ECMP";
  ...
  cmd.AddValue("FlowLaunchEndTime", "End time of the flow launch period",
               FLOW_LAUNCH_END_TIME);
  cmd.AddValue("runMode",
               "Running mode of this simulation: Conga, Conga-flow, Presto, "
               "Weighted-Presto, DRB, FlowBender, ECMP, Clove, DRILL, LetFlow",
               runModeStr);
  ...
  cmd.AddValue("resequenceBuffer", "Whether enabling the resequence buffer",
               resequenceBuffer);
  cmd.AddValue("resequenceInOrderTimer",
               "In order queue timeout in resequence buffer",
               resequenceInOrderTimer);
  cmd.AddValue("resequenceOutOrderTimer",
               "Out order queue timeout in resequence buffer",
               resequenceOutOrderTimer);
  cmd.AddValue("resequenceInOrderSize",
               "In order queue size in resequence buffer",
               resequenceInOrderSize);
  cmd.AddValue("resequenceBufferLog",
               "Whether enabling the resequence buffer logging system",
               resequenceBufferLog);

  cmd.AddValue("asymCapacity",
               "Whether the capacity is asym, which means some link will have "
               "only 1/10 the capacity of others",
               asymCapacity);
  ...
  cmd.AddValue("spineLeafCapacity", "Spine <-> Leaf capacity in Gbps",
               spineLeafCapacity);
  cmd.AddValue("leafServerCapacity", "Leaf <-> Server capacity in Gbps",
               leafServerCapacity);

  cmd.AddValue("TLBMinRTT", "Min RTT used to judge a good path in TLB",
               TLBMinRTT);
  cmd.AddValue("TLBHighRTT", "High RTT used to judge a bad path in TLB",
               TLBHighRTT);
  cmd.AddValue("TLBPoss", "Possibility to change the path in TLB", TLBPoss);
  cmd.AddValue(
      "TLBBetterPathRTT",
      "RTT Threshold used to judge one path is better than another in TLB",
      TLBBetterPathRTT);
  cmd.AddValue("TLBT1", "The path aging time interval in TLB", TLBT1);
  cmd.AddValue("TLBECNPortionLow",
               "The ECN portion used in judging a good path in TLB",
               TLBECNPortionLow);
  cmd.AddValue("TLBRunMode",
               "The running mode of TLB, 0 for minimize counter, 1 for "
               "minimize RTT, 2 for random, 11 for RTT counter, 12 for RTT DRE",
               TLBRunMode);
  cmd.AddValue("TLBProbingEnable", "Whether the TLB probing is enable",
               TLBProbingEnable);
  cmd.AddValue("TLBProbingInterval", "Probing interval for TLB probing",
               TLBProbingInterval);
  cmd.AddValue("TLBSmooth", "Whether the RTT calculation is smooth", TLBSmooth);
  cmd.AddValue("TLBRerouting", "Whether the rerouting is enabled in TLB",
               TLBRerouting);
  cmd.AddValue("TLBDREMultiply", "DRE multiply factor in TLB", TLBDREMultiply);
  cmd.AddValue("TLBS",
               "The S used to judge a whether a flow should change path in TLB",
               TLBS);
  cmd.AddValue("TLBReverseACK",
               "Whether to enable the TLB reverse ACK path selection",
               TLBReverseACK);
  cmd.AddValue("quantifyRTTBase", "The quantify RTT base in TLB",
               quantifyRTTBase);
  cmd.AddValue("TLBFlowletTimeout", "The TLB flowlet timeout",
               TLBFlowletTimeout);

  cmd.AddValue("TcpPause", "Whether TCP will pause in TLB & FlowBender",
               tcpPause);

  cmd.AddValue("applicationPauseThresh",
               "How many packets can pass before we have delay, 0 for disable",
               applicationPauseThresh);
  cmd.AddValue("applicationPauseTime", "The time for a delay, in MicroSeconds",
               applicationPauseTime);

  cmd.AddValue("cloveFlowletTimeout", "Flowlet timeout for Clove",
               cloveFlowletTimeout);
  cmd.AddValue("cloveRunMode",
               "Clove run mode, 1 for edge flowlet, 2 for ECN, 3 for INT (not "
               "yet implemented)",
               cloveRunMode);
  cmd.AddValue("cloveHalfRTT", "Half RTT used in Clove ECN", cloveHalfRTT);
  cmd.AddValue("cloveDisToUncongestedPath",
               "Whether Clove will distribute the weight to uncongested path "
               "(no ECN) or all paths",
               cloveDisToUncongestedPath);

  cmd.AddValue("enableLargeDupAck",
               "Whether to set the ReTxThreshold to a very large value to mask "
               "reordering",
               enableLargeDupAck);

  cmd.AddValue("congaFlowletTimeout", "Flowlet timeout in Conga",
               congaFlowletTimeout);
  ...
  cmd.AddValue(
      "enableRandomDrop",
      "Whether the Spine-0 to other leaves has the random drop problem",
      enableRandomDrop);
  cmd.AddValue("randomDropRate",
               "The random drop rate when the random drop is enabled",
               randomDropRate);

  cmd.AddValue("blackHoleMode",
               "The packet black hole mode, 0 to disable, 1 src, 2 dest, 3 "
               "src/dest pair",
               blackHoleMode);
  cmd.AddValue("blackHoleSrcAddr", "The packet black hole source address",
               blackHoleSrcAddrStr);
  cmd.AddValue("blackHoleSrcMask", "The packet black hole source mask",
               blackHoleSrcMaskStr);
  cmd.AddValue("blackHoleDestAddr", "The packet black hole destination address",
               blackHoleDestAddrStr);
  cmd.AddValue("blackHoleDestMask", "The packet black hole destination mask",
               blackHoleDestMaskStr);

  cmd.AddValue(
      "congaAwareAsym",
      "Whether Conga is aware of the capacity of asymmetric path capacity",
      congaAwareAsym);
  ...
  cmd.AddValue("hostileFlowSize",
               "The size of the flow compromised hosts generate",
               hostileFlowSize);
  cmd.AddValue("attackerOnTime",
               "How long the attacker should send packets until he pauses",
               onTime);
  cmd.AddValue("attackerOffTime",
               "The time for an attacker pause, in MicroSeconds", offTime);
  cmd.AddValue("attackerMode", "Attacker focused on breaking: ECMP, LetFlow",
               attackerModeStr);
  ...
  RunMode runMode =
      parseRunMode(runModeStr, LINK_COUNT, asymCapacity, asymCapacity2);

  // This is just reusing the RunMode type, but in the future we can/should set
  // up different attacker modes.
  RunMode attackerMode =
      parseRunMode(attackerModeStr, LINK_COUNT, asymCapacity, asymCapacity2);
  ...
  if (resequenceBuffer) {
    ...
    Config::SetDefault("ns3::TcpSocketBase::ResequenceBuffer",
                       BooleanValue(true));
    Config::SetDefault("ns3::TcpResequenceBuffer::InOrderQueueTimerLimit",
                       TimeValue(MicroSeconds(resequenceInOrderTimer)));
    Config::SetDefault("ns3::TcpResequenceBuffer::SizeLimit",
                       UintegerValue(resequenceInOrderSize));
    Config::SetDefault("ns3::TcpResequenceBuffer::OutOrderQueueTimerLimit",
                       TimeValue(MicroSeconds(resequenceOutOrderTimer)));
  }

  if (runMode == TLB) {
    NS_LOG_INFO("Enabling TLB");
    Config::SetDefault("ns3::TcpSocketBase::TLB", BooleanValue(true));
    Config::SetDefault("ns3::TcpSocketBase::TLBReverseACK",
                       BooleanValue(TLBReverseACK));
    Config::SetDefault("ns3::Ipv4TLB::MinRTT",
                       TimeValue(MicroSeconds(TLBMinRTT)));
    Config::SetDefault("ns3::Ipv4TLB::HighRTT",
                       TimeValue(MicroSeconds(TLBHighRTT)));
    Config::SetDefault("ns3::Ipv4TLB::BetterPathRTTThresh",
                       TimeValue(MicroSeconds(TLBBetterPathRTT)));
    ...
    Config::SetDefault("ns3::Ipv4TLB::ECNPortionLow",
                       DoubleValue(TLBECNPortionLow));
    Config::SetDefault("ns3::Ipv4TLB::RunMode", UintegerValue(TLBRunMode));
    Config::SetDefault("ns3::Ipv4TLBProbing::ProbeInterval",
                       TimeValue(MicroSeconds(TLBProbingInterval)));
    ...
    Config::SetDefault("ns3::Ipv4TLB::DREMultiply",
                       UintegerValue(TLBDREMultiply));
    Config::SetDefault("ns3::Ipv4TLB::S", UintegerValue(TLBS));
    Config::SetDefault("ns3::Ipv4TLB::QuantifyRttBase",
                       TimeValue(MicroSeconds(quantifyRTTBase)));
    Config::SetDefault("ns3::Ipv4TLB::FlowletTimeout",
                       TimeValue(MicroSeconds(TLBFlowletTimeout)));
  }

  if (runMode == Clove) {
    NS_LOG_INFO("Enabling Clove");
    Config::SetDefault("ns3::TcpSocketBase::Clove", BooleanValue(true));
    Config::SetDefault("ns3::Ipv4Clove::FlowletTimeout",
                       TimeValue(MicroSeconds(cloveFlowletTimeout)));
    Config::SetDefault("ns3::Ipv4Clove::RunMode", UintegerValue(cloveRunMode));
    Config::SetDefault("ns3::Ipv4Clove::HalfRTT",
                       TimeValue(MicroSeconds(cloveHalfRTT)));
    Config::SetDefault("ns3::Ipv4Clove::DisToUncongestedPath",
                       BooleanValue(cloveDisToUncongestedPath));
  }
  ...
  if (runMode == CONGA || runMode == CONGA_FLOW || runMode == CONGA_ECMP) {
    ...
    if (runMode == DRB) {
      Config::SetDefault("ns3::Ipv4DrbRouting::Mode",
                         UintegerValue(0));  // Per dest
    } else {
      Config::SetDefault("ns3::Ipv4DrbRouting::Mode",
                         UintegerValue(1));  // Per flow
    }
  ...
  if (transportProt.compare("DcTcp") == 0) {
    tc.SetRootQueueDisc("ns3::RedQueueDisc", "MinTh",
                        DoubleValue(RED_QUEUE_MARKING * PACKET_SIZE), "MaxTh",
                        DoubleValue(RED_QUEUE_MARKING * PACKET_SIZE));
  }

  NS_LOG_INFO("Configuring servers");
  // Setting servers
  p2p.SetDeviceAttribute("DataRate",
                         DataRateValue(DataRate(LEAF_SERVER_CAPACITY)));
  p2p.SetChannelAttribute("Delay", TimeValue(LINK_LATENCY));
  if (transportProt.compare("Tcp") == 0) {
    p2p.SetQueue("ns3::DropTailQueue", "MaxPackets",
                 UintegerValue(BUFFER_SIZE));
  } else {
  ...
  for (int i = 0; i < LEAF_COUNT; i++) {
    ...
    for (int j = 0; j < SERVER_COUNT; j++) {
      ...
      NodeContainer nodeContainer =
          NodeContainer(leaves.Get(i), servers.Get(serverIndex));
      NetDeviceContainer netDeviceContainer = p2p.Install(nodeContainer);

      if (transportProt.compare("DcTcp") == 0) {
        NS_LOG_INFO("Install RED Queue for leaf: " << i
                                                   << " and server: " << j);
      ...
      NS_LOG_INFO("Leaf - "
                  << i << " is connected to Server - " << j << " with address "
      ...
      if (runMode == TLB) {
        for (int k = 0; k < SERVER_COUNT * LEAF_COUNT; k++) {
          ...
          // NS_LOG_INFO ("Configuring TLB with " << k << "'s server, inserting
          // server: " << j << " under leaf: " << i);
        }
      }
  ...
  // Setting up switches
  p2p.SetDeviceAttribute("DataRate",
                         DataRateValue(DataRate(SPINE_LEAF_CAPACITY)));
  std::set<std::pair<uint32_t, uint32_t> >
      asymLink;  // set< (A, B) > Leaf A -> Spine B is asymmetric
  ...
  for (int i = 0; i < LEAF_COUNT; i++) {
    ...
    for (int j = 0; j < SPINE_COUNT; j++) {
      for (int l = 0; l < LINK_COUNT; l++) {
        ...
        if (asymCapacity &&
            static_cast<uint32_t>(rand() % 100) < asymCapacityPoss) {
          isAsymCapacity = true;
        }
        ...
        p2p.SetDeviceAttribute("DataRate",
                               DataRateValue(DataRate(spineLeafCapacity)));
        ipv4.NewNetwork();

        NodeContainer nodeContainer =
            NodeContainer(leaves.Get(i), spines.Get(j));
        NetDeviceContainer netDeviceContainer = p2p.Install(nodeContainer);
        if (transportProt.compare("DcTcp") == 0) {
          NS_LOG_INFO("Install RED Queue for leaf: " << i
                                                     << " and spine: " << j);
          if (enableRandomDrop) {
            if (j == 0) {
              Config::SetDefault("ns3::RedQueueDisc::DropRate",
                                 DoubleValue(0.0));
              tc.Install(netDeviceContainer.Get(0));  // Leaf to Spine Queue
              Config::SetDefault("ns3::RedQueueDisc::DropRate",
                                 DoubleValue(randomDropRate));
            ...
            if (j == 0) {
              Config::SetDefault("ns3::RedQueueDisc::BlackHole",
                                 UintegerValue(0));
              tc.Install(netDeviceContainer.Get(0));  // Leaf to Spine Queue
              Config::SetDefault("ns3::RedQueueDisc::BlackHole",
                                 UintegerValue(blackHoleMode));
              tc.Install(netDeviceContainer.Get(1));  // Spine to Leaf Queue
              Ptr<TrafficControlLayer> tc =
                  netDeviceContainer.Get(1)
                      ->GetNode()
                      ->GetObject<TrafficControlLayer>();
              Ptr<QueueDisc> queueDisc =
                  tc->GetRootQueueDiscOnDevice(netDeviceContainer.Get(1));
              Ptr<RedQueueDisc> redQueueDisc =
                  DynamicCast<RedQueueDisc>(queueDisc);
              redQueueDisc->SetBlackHoleSrc(blackHoleSrcAddr, blackHoleSrcMask);
              redQueueDisc->SetBlackHoleDest(blackHoleDestAddr,
                                             blackHoleDestMask);
            } else {
              Config::SetDefault("ns3::RedQueueDisc::BlackHole",
                                 UintegerValue(0));
        ...
        Ipv4InterfaceContainer ipv4InterfaceContainer =
            ipv4.Assign(netDeviceContainer);
        NS_LOG_INFO("Leaf - "
                    << i << " is connected to Spine - " << j << " with address "
        ...
        if (runMode == CONGA || runMode == CONGA_FLOW ||
            runMode == CONGA_ECMP) {
          // For each conga leaf switch, routing entry to route the packet to
          // OTHER leaves should be added
          for (int k = 0; k < LEAF_COUNT; k++) {
            if (k != i) {
              congaRoutingHelper
                  .GetCongaRouting(leaves.Get(i)->GetObject<Ipv4>())
                  ->AddRoute(leafNetworks[k], Ipv4Mask("255.255.255.0"),
                             netDeviceContainer.Get(0)->GetIfIndex());
            }
          }

          // For each conga spine switch, routing entry to THIS leaf switch
          // should be added
          Ptr<Ipv4CongaRouting> congaSpine = congaRoutingHelper.GetCongaRouting(
              spines.Get(j)->GetObject<Ipv4>());
          ...
          congaSpine->AddRoute(leafNetworks[i], Ipv4Mask("255.255.255.0"),
                               netDeviceContainer.Get(1)->GetIfIndex());
          ...
          if (isAsymCapacity) {
            ...
            Ptr<Ipv4CongaRouting> congaLeaf =
                congaRoutingHelper.GetCongaRouting(
                    leaves.Get(i)->GetObject<Ipv4>());
        ...
        if (runMode == DRILL) {
          // For each drill leaf switch, routing entry to route the packet to
          // OTHER leaves should be added
          for (int k = 0; k < LEAF_COUNT; k++) {
            if (k != i) {
              drillRoutingHelper
                  .GetDrillRouting(leaves.Get(i)->GetObject<Ipv4>())
                  ->AddRoute(leafNetworks[k], Ipv4Mask("255.255.255.0"),
                             netDeviceContainer.Get(0)->GetIfIndex());
            }
          }

          // For each drill spine switch, routing entry to THIS leaf switch
          // should be added
          Ptr<Ipv4DrillRouting> drillSpine = drillRoutingHelper.GetDrillRouting(
              spines.Get(j)->GetObject<Ipv4>());
          drillSpine->AddRoute(leafNetworks[i], Ipv4Mask("255.255.255.0"),
                               netDeviceContainer.Get(1)->GetIfIndex());
        }

        if (runMode == LetFlow) {
          // For each LetFlow leaf switch, routing entry to route the packet to
          // OTHER leaves should be added
          for (int k = 0; k < LEAF_COUNT; k++) {
            if (k != i) {
              letFlowRoutingHelper
                  .GetLetFlowRouting(leaves.Get(i)->GetObject<Ipv4>())
                  ->AddRoute(leafNetworks[k], Ipv4Mask("255.255.255.0"),
                             netDeviceContainer.Get(0)->GetIfIndex());
            }
          }

          // For each LetFlow spine switch, routing entry to THIS leaf switch
          // should be added
          Ptr<Ipv4LetFlowRouting> letFlowSpine =
              letFlowRoutingHelper.GetLetFlowRouting(
                  spines.Get(j)->GetObject<Ipv4>());
          letFlowSpine->AddRoute(leafNetworks[i], Ipv4Mask("255.255.255.0"),
                                 netDeviceContainer.Get(1)->GetIfIndex());
  ...
  if (runMode == ECMP || runMode == PRESTO || runMode == WEIGHTED_PRESTO ||
      runMode == DRB || runMode == FlowBender || runMode == TLB ||
      runMode == Clove) {
    ...
    for (int i = 0; i < LEAF_COUNT; i++) {
      for (int j = 0; j < SERVER_COUNT; j++) {
        for (int k = 0; k < SPINE_COUNT; k++) {
          int serverIndex = i * SERVER_COUNT + j;
          Ptr<Ipv4DrbRouting> drbRouting = drbRoutingHelper.GetDrbRouting(
              servers.Get(serverIndex)->GetObject<Ipv4>());
          if (runMode == DRB) {
            drbRouting->AddPath(leafToSpinePath[std::make_pair(i, k)]);
          } else if (runMode == WEIGHTED_PRESTO) {
            // If the capacity of a uplink is reduced, the weight should be
            // reduced either
            if (asymLink.find(std::make_pair(i, k)) != asymLink.end()) {
              drbRouting->AddWeightedPath(
                  PRESTO_RATIO * 0.2, leafToSpinePath[std::make_pair(i, k)]);
            } else {
              // Check whether the spine down to the leaf is reduced and add the
              // exception
              std::set<Ipv4Address> exclusiveIPs;
              for (int l = 0; l < LEAF_COUNT; l++) {
                if (asymLink.find(std::make_pair(k, l)) != asymLink.end()) {
                  for (int m = l * SERVER_COUNT;
                       m < l * SERVER_COUNT + SERVER_COUNT; m++) {
                    Ptr<Node> destServer = servers.Get(m);
                    Ptr<Ipv4> ipv4 = destServer->GetObject<Ipv4>();
                    Ipv4InterfaceAddress destInterface = ipv4->GetAddress(1, 0);
                    Ipv4Address destAddress = destInterface.GetLocal();
                    drbRouting->AddWeightedPath(
                        destAddress, PRESTO_RATIO * 0.2,
                        leafToSpinePath[std::make_pair(i, k)]);
                    exclusiveIPs.insert(destAddress);
                  }
                }
              }
              drbRouting->AddWeightedPath(PRESTO_RATIO,
                                          leafToSpinePath[std::make_pair(i, k)],
                                          exclusiveIPs);
            }
          } else {
            drbRouting->AddPath(PRESTO_RATIO,
                                leafToSpinePath[std::make_pair(i, k)]);
          }
        }
      }
      ...
      for (int j = 0; j < SERVER_COUNT; j++) {
        ...
        for (int k = 0; k < SPINE_COUNT; k++) {
          ...
          for (int l = 0; l < LEAF_COUNT; l++) {
            ...
            int newPath =
                spineToLeafPath[std::make_pair(k, l)] * pathBase + path;
            Ptr<Ipv4Clove> clove =
                servers.Get(serverIndex)->GetObject<Ipv4Clove>();
  ...
  if (runMode == TLB) {
      ...
      for (int j = 0; j < SERVER_COUNT; j++) {
        ...
        for (int k = 0; k < SPINE_COUNT; k++) {
          ...
          for (int l = 0; l < LEAF_COUNT; l++) {
            ...
            // NS_LOG_INFO ("Configuring server: " << serverIndex << " to leaf:
            // " << l << " with path: " << newPath);
          }
        }
      }
    ...
    if (runMode == TLB && TLBProbingEnable) {
      ...
      for (int i = 0; i < SERVER_COUNT * LEAF_COUNT; i++) {
        // The i th server under one leaf is used to probe the leaf i by
        // contacting the i th server under that leaf
        ...
        if (serverIndexUnderLeaf < LEAF_COUNT) {
          ...
          // NS_LOG_INFO ("Server: " << i << " is going to probe server: " <<
          // serverBeingProbed);
          int leafIndex = i / SERVER_COUNT;
          for (int j = leafIndex * SERVER_COUNT;
               j < leafIndex * SERVER_COUNT + SERVER_COUNT; j++) {
            if (i == j) {
              continue;
            }
            probing->AddBroadCastAddress(serverAddresses[j]);
            // NS_LOG_INFO ("Server:" << i << " is going to broadcast to server:
            // " << j);
          }
          probing->StartProbe();
  ...
  double oversubRatio =
      static_cast<double>(SERVER_COUNT * LEAF_SERVER_CAPACITY) /
  ...
  double requestRate = load * LEAF_SERVER_CAPACITY * SERVER_COUNT /
                       oversubRatio / (8 * avg_cdf(cdfTable)) / SERVER_COUNT;
  ...
  for (int fromLeafId = 0; fromLeafId < LEAF_COUNT; fromLeafId++) {
    install_applications(fromLeafId, servers, requestRate, cdfTable, flowCount,
                         ...
                         hostileFlows, attackerProt, attackerApp);
  }

  NS_LOG_INFO("Total flow: " << flowCount);

  NS_LOG_INFO("Actual average flow size: "
              << static_cast<double>(totalFlowSize) / flowCount);
  ...
  if (linkMonitorEnabled) {
    ...
    //   leafLinkProbe->SetCheckTime(Seconds(0.0005));
  ...
  tlbBibleFilename2 << id << "-1-large-load-" << LEAF_COUNT << "X"
                    << SPINE_COUNT << "-" << load << "-" << transportProt
                    << "-";
  rbTraceFilename << id << "-1-large-load-" << LEAF_COUNT << "X" << SPINE_COUNT
                  << "-" << load << "-" << transportProt << "-";
  ...
  if (runMode == CONGA) {
    ...
    flowMonitorFilename << "flow-bender-" << flowBenderT << "-" << flowBenderN
                        << "-simulation-";
    linkMonitorFilename << "flow-bender-" << flowBenderT << "-" << flowBenderN
                        << "-simulation-";
  } else if (runMode == TLB) {
    flowMonitorFilename << "tlb-" << TLBHighRTT << "-" << TLBMinRTT << "-"
                        << TLBBetterPathRTT << "-" << TLBPoss << "-" << TLBS
                        << "-" << TLBT1 << "-" << TLBProbingInterval << "-"
                        << TLBSmooth << "-" << TLBRerouting << "-"
                        << quantifyRTTBase << "-";
    linkMonitorFilename << "tlb-" << TLBHighRTT << "-" << TLBMinRTT << "-"
                        << TLBBetterPathRTT << "-" << TLBPoss << "-" << TLBS
                        << "-" << TLBT1 << "-" << TLBProbingInterval << "-"
                        << TLBSmooth << "-" << TLBRerouting << "-"
                        << quantifyRTTBase << "-";
    tlbBibleFilename << "tlb-" << TLBHighRTT << "-" << TLBMinRTT << "-"
                     << TLBBetterPathRTT << "-" << TLBPoss << "-" << TLBS << "-"
                     << TLBT1 << "-" << TLBProbingInterval << "-" << TLBSmooth
                     << "-" << TLBRerouting << "-" << quantifyRTTBase << "-";
    tlbBibleFilename2 << "tlb-" << TLBHighRTT << "-" << TLBMinRTT << "-"
                      << TLBBetterPathRTT << "-" << TLBPoss << "-" << TLBS
                      << "-" << TLBT1 << "-" << TLBProbingInterval << "-"
                      << TLBSmooth << "-" << TLBRerouting << "-"
                      << quantifyRTTBase << "-";
  } else if (runMode == Clove) {
    flowMonitorFilename << "clove-" << cloveRunMode << "-"
                        << cloveFlowletTimeout << "-" << cloveHalfRTT << "-"
                        << cloveDisToUncongestedPath << "-";
    linkMonitorFilename << "clove-" << cloveRunMode << "-"
                        << cloveFlowletTimeout << "-" << cloveHalfRTT << "-"
                        << cloveDisToUncongestedPath << "-";
  } else if (runMode == DRILL) {
  ...
  if (resequenceBuffer) {
    flowMonitorFilename << "rb-" << resequenceInOrderSize << "-"
                        << resequenceInOrderTimer << "-"
                        << resequenceOutOrderTimer;
    linkMonitorFilename << "rb-" << resequenceInOrderSize << "-"
                        << resequenceInOrderTimer << "-"
                        << resequenceOutOrderTimer;
    rbTraceFilename << "rb-";
  }
  ...
  if (applicationPauseThresh > 0) {
    flowMonitorFilename << "p" << applicationPauseThresh << "-"
                        << applicationPauseTime << "-";
    linkMonitorFilename << "p" << applicationPauseThresh << "-"
                        << applicationPauseTime << "-";
    tlbBibleFilename << "p" << applicationPauseThresh << "-"
                     << applicationPauseTime << "-";
    tlbBibleFilename2 << "p" << applicationPauseThresh << "-"
                      << applicationPauseTime << "-";
  }
  ...
  if (runMode == TLB) {
    ...
    std::ofstream out(tlbBibleFilename.str().c_str(),
                      std::ios::out | std::ios::app);
    out << Ipv4TLB::GetLogo();
    std::ofstream out2(tlbBibleFilename2.str().c_str(),
                       std::ios::out | std::ios::app);
 ...
 std::set<int>::iterator it = compromised.begin();
  ...
  while (it != compromised.end()) {
    ...
    serverAddresses[(*it)].Print(flowMonitorString);
    flowMonitorString << "\"/>\n";
    it++;
  }
